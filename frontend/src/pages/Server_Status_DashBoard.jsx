...
const App = () => {
  ...
  useEffect(() => {
    ...
    // 서버 상태 불러오기
    axios.get('http://localhost:5173/api/server-status')
      .then(response => {
      ...
      <div className="flex flex-col flex-1">
        <main className="flex-1 p-6 overflow-auto" style={{ fontFamily: 'Pretendard-Regular' }}>
          ...
          <div className="flex items-center space-x-2 mb-4 gap-1">
            ...
            <div>
              <h1 className="text-2xl font-semibold mb-1">서버 현황</h1>
              <h1 className="text-base font-normal">서버의 상태를 한눈에 보고 진단해보세요!</h1>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 mb-8">
            <div className="p-4 bg-white rounded shadow">
              ...
              <p>서버 네트워크 상태: {serverStatus.networkStatus}</p>
              <p>평균 트래픽: {serverStatus.averageTraffic}</p>
              <p>설정된 네트워크 대역폭: 1Gbps</p>
            </div>
            <div className="p-4 bg-white rounded shadow">
              <h2 className="text-lg font-semibold mb-2">현재 서버 상태</h2>
              <p>메인 서버 상태: {serverStatus.mainServer}</p>
              <p>프록시 서버 상태: {serverStatus.proxyServer}</p>
              <p>데이터베이스 서버 상태: {serverStatus.databaseServer}</p>
